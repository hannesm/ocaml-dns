...
let find_or_generate_key key_filename bits seed =
  ...
  | true ->
    ...
    Rresult.R.reword_error
      (function `Parse msg -> `Msg ("while parsing private key file: " ^ msg))
...
let jump _ server_ip port (keyname, zone, dnskey) hostname csr key seed bits cert force =
  ...
  (Bos.OS.File.exists csr_filename >>= function
    | true ->
      ...
      Rresult.R.reword_error
        (function `Parse msg -> `Msg ("while parsing csr: " ^ msg))
  ...
  (Bos.OS.File.exists cert_filename >>= function
    | true ->
      ...
      Rresult.R.reword_error
        (function `Parse msg -> `Msg ("while parsing certificate: " ^ msg))
